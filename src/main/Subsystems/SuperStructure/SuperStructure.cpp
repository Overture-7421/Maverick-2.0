...
SuperStructure::SuperStructure() {
	...
	lowerRightMotor.configureMotionMagic(ConstantsSS::LowerCruiseVelocity, ConstantsSS::LowerCruiseAcceleration, 0.0_tr_per_s_cu);
	upperMotor.configureMotionMagic(ConstantsSS::UpperCruiseVelocity, ConstantsSS::UpperCruiseAcceleration, 0.0_tr_per_s_cu);
}
...
void SuperStructure::setToAngle(units::degree_t lowerAngle, units::degree_t upperAngle) {
	...
	lowerRightMotor.SetControl(lowerVoltage.WithPosition(lowerAngle).WithFeedForward(armFeedForward.Calculate(turns, turnsPerSecond)).WithEnableFOC(true).WithSlot(0));
	upperMotor.SetControl(upperVoltage.WithPosition(upperAngle).WithFeedForward(wristFeedForward.Calculate(wristTurns, wristTurnsPerSecond)).WithEnableFOC(true).WithSlot(0));
};
